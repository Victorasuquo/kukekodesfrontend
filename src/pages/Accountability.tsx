...
export default function Accountability() {
  ...
  const refresh = async () => { try { setError(null); const next = await api.getAccountabilityStatus(); setStatus(next); if (next.status === 'active') setCluster(await api.getAccountabilityCluster()); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to load accountability status'); } finally { setLoading(false); } };
